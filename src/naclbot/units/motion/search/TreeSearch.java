...
public class TreeSearch extends GlobalVars {
    ...
    public static ArrayList<TreeInfo> getNearbyNeutTrees() {
	TreeInfo[] treeList = rc.senseNearbyTrees();
	ArrayList<TreeInfo> viableList = new ArrayList<TreeInfo>();
	for (int i = 0; i < treeList.length; i++) {
	    if (treeList[i].team == Team.NEUTRAL) {
		viableList.add(treeList[i]);
	...
	for (int i = 0; i < treeList.length; i++) {
	    if (treeList[i].getContainedBullets() > 0 && treeList[i].getTeam() != rc.getTeam().opponent()) {
