...
public class ArchonBotOld extends GlobalVars {
    ...
    public static int getGardenerLimit(int roundNumber) throws GameActionException{
    	...
    	// Make exponential
    		...
    		return (3 + (int)Math.ceil(roundNumber / 100));
    	}
    	else{
    		return (3 + (int)Math.ceil(roundNumber / 75));    	
    	}
    }    
...
//                System.out.println("Cur # Gardeners: " + gardenerCount);
                ...
                if (!crowded) {
                	if ((rc.getTreeCount() > 4*gardenerCount) || ((soldierCount + lumberjackCount) > 3*gardenerCount)){
