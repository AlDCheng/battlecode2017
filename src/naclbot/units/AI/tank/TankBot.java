...
public class TankBot extends GlobalVars {
	...
	// The total number of tanks in active service
	private static int currentNumberofTanks;
	
	// Boolean to store whether or not the tank current has orders to go somewhere....
	...
	// Direction at which the tank traveled last
	...
	// The ID of the robot the tank is currently tracking and its information
    ...
    // Variable to determine in which direction the tank will rotate about a particular robot when tracking it....
    ...
    public static void init() throws GameActionException{
    	...
    	// SYSTEM CHECK - See if the tank has initialized...    	
        ...
        // Initialize tank so that it does not have any commands initially;
        isCommanded = false;

        // Get own tankNumber - important for broadcasting 
    ...
    private static MapLocation move(RobotInfo[] enemyRobots, MapLocation desiredMove) throws GameActionException{
    	...
    	if(trackID == -1){    		
    		...
    		if (trackedRobot != null){
        		...
        		// System.out.println("The tank has noticed the enemy Robot with ID: " + trackID);
    	...
    	// If the robot is actually currently tracking something
    		...
    		// If the tank is currently not commanded to go anywhere... follow the robot in question
    		if(!isCommanded){
